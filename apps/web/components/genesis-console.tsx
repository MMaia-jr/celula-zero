...
export function GenesisConsole({ records, gatewayReady }: { records: GenesisRecord[]; gatewayReady: boolean }) {
  return (
    <section className="content-block">
      ...
      <form className="project-form" action={recordGenesisMessage}>
        <label htmlFor="genesis-message">Mensagem</label>
        <textarea id="genesis-message" name="message" rows={5} maxLength={16000} required placeholder="Registre o que você quer preservar…" />
        <button className="button button-primary" type="submit">Preservar como Original Record</button>
      </form>
