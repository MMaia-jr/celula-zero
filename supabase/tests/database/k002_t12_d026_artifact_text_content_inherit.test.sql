...
-- ---------------------------------------------------------------------------
-- D026 content-inheritance fixture: immutable text payloads for the same three
-- Artifacts used by the canonical parent-Artifact visibility matrix.
-- ---------------------------------------------------------------------------

insert into public.artifact_text_contents(artifact_id,content) values
  ('95000000-0000-4000-8000-000000000211','D026 synthetic PRIVATE text content'),
  ('95000000-0000-4000-8000-000000000212','D026 synthetic PARTIES text content'),
  ('95000000-0000-4000-8000-000000000213','D026 synthetic PROJECT text content');
...
-- ---------------------------------------------------------------------------
-- D026 artifact_text_contents inheritance regression.
-- Parent Artifact semantics are already asserted above. This extension requires
-- content reads to match the parent Artifact exactly across the same 12 cells.
-- ---------------------------------------------------------------------------

create temporary table t12_content_observed(
  profile_label text not null,
  visibility text not null,
  visible_count bigint not null,
  primary key(profile_label,visibility)
);
grant select, insert on t12_content_observed to authenticated;

-- AUTHOR
select set_config('request.jwt.claim.sub','95000000-0000-4000-8000-000000000001',true);
set local role authenticated;
insert into t12_content_observed
select 'AUTHOR',v.visibility,count(tc.artifact_id)::bigint
from (values
  ('PRIVATE','95000000-0000-4000-8000-000000000211'::uuid),
  ('PARTIES','95000000-0000-4000-8000-000000000212'::uuid),
  ('PROJECT','95000000-0000-4000-8000-000000000213'::uuid)
) v(visibility,artifact_id)
left join public.artifact_text_contents tc on tc.artifact_id=v.artifact_id
group by v.visibility;
reset role;
...
set local role authenticated;
insert into t12_content_observed
select 'COUNTERPARTY',v.visibility,count(tc.artifact_id)::bigint
from (values
  ('PRIVATE','95000000-0000-4000-8000-000000000211'::uuid),
  ('PARTIES','95000000-0000-4000-8000-000000000212'::uuid),
  ('PROJECT','95000000-0000-4000-8000-000000000213'::uuid)
...
set local role authenticated;
insert into t12_content_observed
select 'STEWARD',v.visibility,count(tc.artifact_id)::bigint
from (values
  ('PRIVATE','95000000-0000-4000-8000-000000000211'::uuid),
  ('PARTIES','95000000-0000-4000-8000-000000000212'::uuid),
  ('PROJECT','95000000-0000-4000-8000-000000000213'::uuid)
...
set local role authenticated;
insert into t12_content_observed
select 'OUTSIDER',v.visibility,count(tc.artifact_id)::bigint
from (values
  ('PRIVATE','95000000-0000-4000-8000-000000000211'::uuid),
  ('PARTIES','95000000-0000-4000-8000-000000000212'::uuid),
  ('PROJECT','95000000-0000-4000-8000-000000000213'::uuid)
...
create temporary table t12_content_expected(
  profile_label text not null,
  visibility text not null,
  expected_visible_count bigint not null,
  primary key(profile_label,visibility)
);
insert into t12_content_expected(profile_label,visibility,expected_visible_count) values
  ('AUTHOR','PRIVATE',1),
  ('AUTHOR','PARTIES',1),
  ('AUTHOR','PROJECT',1),
  ('COUNTERPARTY','PRIVATE',0),
...
select is(
  (select count(*)::bigint from t12_content_observed),
  12::bigint,
  'artifact_text_contents regression produced all 12 audience cells'
);

select is(
  (
    select count(*)::bigint
    from t12_content_observed c
    join t12_observed a
      on a.profile_label=c.profile_label
     and a.visibility=c.visibility
     and a.object_type='ARTIFACT'
    where c.visible_count <> a.visible_count
  ),
  0::bigint,
  'artifact_text_contents read access exactly matches parent Artifact across 12 cells'
);

select is(
  (
    select count(*)::bigint
    from t12_content_expected e
    join t12_content_observed o using(profile_label,visibility)
    where o.visible_count <> e.expected_visible_count
  ),
  0::bigint,
  'artifact_text_contents satisfies D026 across all 12 audience cells'
);

select diag(
  E'CONTENT RLS MATRIX\n' ||
  coalesce((
    select string_agg(
      'CONTENT_MATRIX|' || profile_label || '|' || visibility ||
      '|visible=' || visible_count,
      E'\n'
      order by profile_label,
        case visibility when 'PRIVATE' then 1 when 'PARTIES' then 2 else 3 end
    )
    from t12_content_observed
  ),'')
);

select diag(
  E'CONTENT NORMATIVE DIVERGENCES\n' ||
  coalesce((
    select string_agg(
      'CONTENT_DIVERGENCE|' || e.profile_label || '|' || e.visibility ||
      '|expected=' || e.expected_visible_count || '|actual=' || o.visible_count,
      E'\n'
      order by e.profile_label,
        case e.visibility when 'PRIVATE' then 1 when 'PARTIES' then 2 else 3 end
    )
    ...
    join t12_content_observed o using(profile_label,visibility)
    where o.visible_count <> e.expected_visible_count
  ),'CONTENT_DIVERGENCE|NONE')
);

select diag(
  'CONTENT_POLICY|' || coalesce((
    select regexp_replace(pg_get_expr(polqual,polrelid), E'[\\n\\r]+', ' ', 'g')
    from pg_policy
    where polrelid='public.artifact_text_contents'::regclass
      and polname='artifact_text_contents_read'
  ),'MISSING')
);
